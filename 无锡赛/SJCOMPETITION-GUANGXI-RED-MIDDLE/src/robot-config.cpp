...
const double gear_ratio = 48.0 / 60.0;                     // 底盘电机-�?的齿�?传动比（加速配比就大于1，减速配比就小于1�?
...
double set_convey_belt_spin_time = 0;                // 用在�?动函数convey_belt_spin_for_time�?的参�?
extern bool start_convey_belt_spin_for_time = false; // 用在�?动函数convey_belt_spin_for_time�?的参�?
/*************************************
...
// motor L4 = motor(PORT22, ratio6_1, true);
// motor R1 = motor(PORT1, ratio6_1, false);
// motor R2 = motor(PORT3, ratio6_1, false);
// motor R3 = motor(PORT7, ratio6_1, false);
// motor R4 = motor(PORT22, ratio6_1, false); 

// 铝件
motor L1 = motor(PORT17, ratio6_1, true);
...
// �?升臂
motor lift_armMotorA = motor(PORT6, ratio18_1, true);
motor lift_armMotorB = motor(PORT9, ratio18_1, false);
motor_group lift_arm = motor_group(lift_armMotorA, lift_armMotorB);

// 传送带
motor convey_beltMotorA = motor(PORT8, ratio6_1, true);
motor convey_beltMotorB = motor(PORT11, ratio6_1, false);
motor_group convey_belt = motor_group(convey_beltMotorA, convey_beltMotorB);

// 吸球
motor rollerMotorA = motor(PORT4, ratio18_1, false);
motor rollerMotorB = motor(PORT22, ratio18_1, false);
motor_group roller_group = motor_group(rollerMotorA, rollerMotorB);
// 遥控�?
controller Controller1 = controller(primary);
// 通信天线
vex::message_link AllianceLink(PORT22, "tju1", linkType::worker);
// 里程�?
encoder encoderHorizonal = encoder(Brain.ThreeWirePort.A);
encoder encoderVertical = encoder(Brain.ThreeWirePort.G);
// 导入�?
motor side_bar = motor(PORT22, ratio18_1, false);
// imu�?性传感器
inertial imu = inertial(PORT14);  // �?二个参数要写right

// 气动�?
pwm_out gas_push = pwm_out(Brain.ThreeWirePort.D);
pwm_out gas_lift = pwm_out(Brain.ThreeWirePort.B);
pwm_out gas_hold = pwm_out(Brain.ThreeWirePort.A);

// 距�?�传感器
distance DistanceSensor = distance(PORT22);
// gps
gps GPS_ = gps(PORT22, 0, 0, inches, 0);
// 旋转传感�?
rotation ArmRotationSensor = rotation(PORT18, false);
rotation roller_group_rotation = rotation(PORT21,false);
...
// vision
vision Vision = vision(PORT22, 50, Red1, Blue2);
...
ai::robot_link       link( PORT22, "robot_32456_1", linkType::manager );
#else
