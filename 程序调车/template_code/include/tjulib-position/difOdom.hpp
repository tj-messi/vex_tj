    ...
    public:
        ...
        void OdomRun(){
            ...
            while(true) {         
                ...
                localDeltaPoint.y = CenterRidus*(1-cos(DeltaHeading));
                localDeltaPoint.x = CenterRidus*sin(DeltaHeading);
                // 计算全局坐标中的变化
                globalPoint.y = prevGlobalPoint.y - localDeltaPoint.y*cos(prevGlobalPoint.angle) + localDeltaPoint.x*sin(prevGlobalPoint.angle);
                globalPoint.x = prevGlobalPoint.x + localDeltaPoint.y*sin(prevGlobalPoint.angle) + localDeltaPoint.x*cos(prevGlobalPoint.angle);
                // 更新坐标
                ...
                // 调试信息打印
                //printf("x : %lf cell , y : %lf cell , heading : %lf imu : %lf\n",globalPoint.x / cell ,globalPoint.y / cell , globalPoint.angle, imu.rotation());
