...
namespace tjulib
{
    ...
    public:
        ...
        T GetCircleControlGamma(T targetDeg, T currentAngle, T distance){
            ...
            //  printf("t1:%lf\n",(double)(b/distance));
            //  printf("sin:%lf\n",sin((delta_theta/ 180 * 3.14159 *0.5 )));
            // 计算算子，最后一小段因为很小，直接计算会出现一些问题，因此考虑直接返回0了
            if(std::fabs(delta_theta)<2){
                return 0;
            }else{
                return (double)(b/distance) * (double)sin((delta_theta/ 180 * 3.14159 *0.5 ));
            }
        ...
        void CircleTurnMove(T target_angle, T distance, T maxSpeed, double maxtime_ms, int fwd = 1){
            ...
            double currentAngle = Math::getWrap360(imu.angle());
            ...
            gamma = GetCircleControlGamma(error, 0, distance);
            printf("\ngamma:%lf\n", gamma);
