...
{
    using namespace vex;
    class CurChassis : virtual public ChassisMotionFunc{
    protected:
        T b; // car width
    ...
    public:
        CurChassis(std::vector<vex::motor*> &leftMotors, std::vector<vex::motor*> &rightMotors, 
                Position *odom, pidControl *cur_pid, pidControl *straightline_pid, T car_width)
            : ChassisMotionFunc(leftMotors, rightMotors, straightline_pid, odom), 
            circleControl(cur_pid), b(car_width)
        {};
        ...
        void CircleTurnMove(T target_angle, T distance, T maxSpeed, double maxtime_ms, int fwd = 1){
            ...
            angleWrap(targetDeg, currentAngle);  
            ...
            //  圆弧转向控制
            while (!circleControl->overflag() || (fabs(currentAngle-targetDeg) >= 2)) // If within acceptable distance, PID output is zero.
            {
                ...
                // 震荡回调控制
                if(std::fabs(targetDeg - currentAngle) < circleControl->params->errorThreshold && finalTurnSpeed <= circleControl->params->minSpeed){
                    circleControl->cnt++;
                }
                // 零误差漂移修正
                currentAngle = imu.rotation() - zero_drift_error;
                ...
                finalTurnSpeed = circleControl->pidCalcu(targetDeg, maxSpeed, currentAngle); 
