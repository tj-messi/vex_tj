...
namespace tjulib
{
    ...
    public:
        ...
        void turnToAngle(double angle, T maxSpeed, double maxtime_ms, int fwd){
            ...
            //  || odom->deltaAngle > DELTA_ANGLE_EPO 额外的控制条件
            while ((fabs(currentAngle-targetDeg) > 2) && (totaltime=mytime.time(msec)<maxtime_ms)) // If within acceptable distance, PID output is zero.
            {
                currentAngle = imu.rotation();
                
                angleWrap(targetDeg, currentAngle);  // 获取最近角度，连续值计算
        ...
        void moveInches(T inches, T maxSpeed, double maxtime_ms = 5000, int fwd = 1){
            ...
            while (fabs(finalFwdSpeed) > 5 &&(mytime.time(msec)<maxtime_ms)) // If within acceptable distance, PID output is zero.
            {
    ...
    // 由于是虚类继承，所以只有一份OdomChassis的内存映像
